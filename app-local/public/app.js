...
async function refreshAll() {
  await Promise.all([loadProducts(), refreshSummary(), loadSales(), checkHealth()]);
}
...
async function loadProducts(query = '') {
  ...
  renderProducts();
  renderSearchResults(query ? data.products : []);
}
  ...
  els.salesList.innerHTML = data.sales.map((sale) => `
    <div class="sale-row">
      <div>
        ...
        <div class="meta">${formatDate(sale.created_at)} - ${sale.payment_method}</div>
      </div>
      <span>${sale.status}</span>
    </div>
  `).join('') || '<div class="meta">Sin ventas registradas.</div>';
}
...
function renderCart() {
  ...
  `).join('') || '<div class="cart-item"><span class="meta">Carrito vacio.</span></div>';
